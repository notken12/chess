...
class ResBlock(nn.Module):
    def __init__(
        self, convChannels
    ):  # channels is 256 because we should be feeding the 256 x 8 x 8 into the resBlocks
        super().__init__()
        # We use padding=1 to keep the 8x8 spatial dimension the same, kernel size 3x3 conv filter
        self.conv1 = nn.Conv2d(
            convChannels, convChannels, kernel_size=3, padding=1, bias=False
        )
...
class Representation(nn.Module):
    def __init__(
        self, obsChannels, convChannels
    ):  # obsChannels = 119, convChannels = 256
        super().__init__()

        self.conv1 = nn.Conv2d(
            obsChannels, convChannels, kernel_size=3, padding=1, bias=False
        )
        self.bn1 = nn.BatchNorm2d(convChannels)
        self.relu = nn.ReLU()
...
class Dynamics(nn.Module):
    def __init__(
        self, actionChannels, stateChannels, convChannels
    ):  # actions is now 73 x 8 x 8
        super().__init__()

        self.conv1 = nn.Conv2d(
            actionChannels + stateChannels,
            convChannels,
            kernel_size=3,
            padding=1,
            bias=False,
        )  # convert 329 x 8 x 8 to 256 x 8 x 8
        self.bn1 = nn.BatchNorm2d(convChannels)
        self.relu = nn.ReLU()
        self.resblocks = nn.Sequential(*[ResBlock(convChannels) for _ in range(10)])
        ...
        for r1 in range(8):
            for f2 in range(8):
                for r2 in range(8):
                    ...
                    elif distance in knightMove:  # knight move
