...
def _encode_snapshot(
    board: chess.Board, tensor: torch.Tensor, channel_offset: int
) -> None:
...
def board_to_observation(
    board: chess.Board, history: list[chess.Board]
) -> torch.Tensor:
    """
    ...
    for snapshot_idx, snapshot_board in enumerate(history[:NUM_SNAPSHOTS]):
        _encode_snapshot(snapshot_board, tensor, snapshot_idx * CHANNELS_PER_SNAPSHOT)
